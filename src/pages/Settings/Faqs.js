...
const Banner = () => {
  ...
  const addbenners = () => {
    ...
    axios
      .post(URLS.UpdateFaqs, dataArray, {
  ...
  const deletebenners = data => {
    var token = datas
    var remid = { question_id: data._id }

    axios
      .post(URLS.deleteFaqs, remid, {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then(
        res => {
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <Breadcrumbs title="Whatnot" breadcrumbItem="FAQ'S" />
          <Row>
            <Col md={4}>
              <Card>
            ...
            <Col md={8}>
              <Card>
                <CardHeader className="bg-white">
                  <CardTitle>FAQ'S List</CardTitle>
                </CardHeader>
                {isLoading == true ? (
                  <>
                    <div
