...
function TermsConditions() {
  ...
  const GetSettings = () => {
    ...
    axios
      ...
      .then(res => {
        setform(res.data.policies.termsCondition)
        setIsLoading(false)
      })
  }
  ...
  const changstatus = () => {
    ...
    axios
      .post(URLS.UpdateSettings, dataArray, {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then(
        res => {
          if (res.status === 200) {
            toast("Terms & Conditions has been updated successfully")
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <Breadcrumbs title="Whatnot" breadcrumbItem="Terms & Conditions" />
          <Row>
            <Col md={12}>
              <Card>
